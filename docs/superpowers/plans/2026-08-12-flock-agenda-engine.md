...
> **EXECUTED 2026-08-18** (branch claude/sophos-install-sheep-tasks-5y1wv2, patron syl) — Phase 1 complete, all task boxes ticked. Dated adaptations from post-plan reality: famacha key normalized to `score` (engine reads canonical-first, legacy fallback kept as defense-in-depth); typed health-event log (MCS-26, 2026-08-18) added as second treatment source with explicit recorded locks winning over re-derivation and (animal,date,drug) dedupe; 'VB' vitamin-B shorthand added to benign vocabulary (present in real treatment strings); deceased animals excluded from locks; hand-kept withdrawal_watch surfaced as flock-level items until MCS-38 retires it. Phases 2-6 remain future plans against open-claw-stuff.
...
- [x] **Step 5: Commit** — `git add scripts/lib/flock_agenda.py scripts/test_flock_agenda.py && git commit -m "feat(agenda): date parsing + treatment classification"`

### Task 2: withdrawal locks + unknown-drug flags

**Files:** Modify `scripts/lib/flock_agenda.py`; extend `scripts/test_flock_agenda.py`.

- [x] **Step 1: Failing tests (append before the failures check)**
...
- [x] **Step 2: Run — expect ImportError on `withdrawal_items`**
- [x] **Step 3: Implement**
...
- [x] **Step 4: Run tests — pass** · **Step 5: Commit** `feat(agenda): withdrawal locks + unknown-drug flags`
...
- [x] **Step 2: Run — ImportError expected** · **Step 3: Implement**
...
- [x] **Step 4: Run tests — pass** · **Step 5: Commit** `feat(agenda): FECRT windows, FAMACHA rechecks, anomaly items`
...
- [x] **Step 2: Run — ImportError** · **Step 3: Implement**
...
- [x] **Step 4: Run `python3 scripts/test_flock_agenda.py` (all pass) AND the live smoke:**
...
- [x] **Step 5: Run the full suite** — `python3 scripts/test_flock_agenda.py && python3 scripts/test_pen_history.py && python3 scripts/test_validate_flock.py && python3 scripts/validate_flock.py`
- [x] **Step 6: Commit** `feat(agenda): assembly + CLI — the household integration feed`

### Task 5: ledger + HLS closeout

- [x] Mark the plan/agenda work in `docs/UPGRADE-LEDGER.md` (MCS-7/11/30 rows gain "engine shipped" note), commit `[no-reasoning]`, push, and return the HLS task for quorum.
