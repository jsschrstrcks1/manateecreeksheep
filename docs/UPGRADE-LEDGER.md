...
avoidance-costs-forage tradeoff, polygenic/MHC — no single letter code, consistent with MCS-32's
Tier-2 stance). The mechanism detail is context, not action; **three actionable deltas adopted:**
(1) **Periparturient egg rise → new row MCS-33** (the big one — connects MCS-17's lambing calendar
to parasite epidemiology and adds *maternal resistance* as MCS-32's fourth parasite bar). (2)
**Nutrition funds immunity** — protein is the budget for antibodies/mucus/gut repair; late-
gestation/lactation/growing animals are squeezed hardest → strengthens MCS-23's case and adds the
caveat that Tier-2 trait expression is resource-dependent ("resistant in a good year, less so in
drought or raising twins") — so FEC/FAMACHA observations should carry nutritional/exposure context,
which our contemporary-grouping partially does and the observation record should complete. (3) Its
closing discipline — "a sheep that never looks ill may be resistant, resilient, lightly exposed,
recently treated, older, or well fed" — is the observation-context rule MCS-30's guardrails now
carry. Everything else confirmed existing rows.
