...
author: skynet2 (flock superset 2026-08-08 = canonical v1.4.1 + Manatee Creek flock domain; superset of ocs-work v1.1.0 four-layer + hermes multi-agent/pitfalls lineage; adopted from InTheWake CAREFUL patterns)
...
> **Superset copy — Manatee Creek Sheep.** This file is canonical `careful-not-clever` v1.4.1
> **in full**, plus the flock-domain sections this repo has always carried (now in
> [§Flock domain](#flock-domain--manatee-creek-sheep)). It is registered `policy: variant` in
> `open-claw-stuff/tools/skill-sync/skills-manifest.json` and must **never** be converged by
> overwrite in either direction: canonical would delete the flock knowledge, and this file would
> delete nothing but is not the source. Upstream doctrine changes come DOWN by merge; flock
> knowledge never goes up. Before 2026-08-08 this copy was 54 lines and carried none of the four
> layers, the economy ladder, the confidence scale, the destructive-command refusals, or the
> pitfall catalog.
...
## Flock domain — Manatee Creek Sheep

*Repo-specific. Never present in canonical, never propagated upward. This is Layer 1 (Execution)
instantiated for flock records — it does not replace the four layers above, it applies them.*
